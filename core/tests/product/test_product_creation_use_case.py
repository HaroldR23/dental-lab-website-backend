...
def test_create_product_should_raise_an_exception_when_there_is_a_product_with_same_name(
    ...
    error_msg = f"The Product with the name {product_creation_request.name} already exists."
...
def test_create_product_should_raise_an_exception_when_something_went_wrong_during_the_creation(
    ...
    # Act
    with patch.object(MemoryProductRepository, "create", side_effect=ProductRepositoryException(method="create")):
